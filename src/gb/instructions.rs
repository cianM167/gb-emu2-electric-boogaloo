...
pub enum Operands {
    ...
    BitMem(u8, MemTarget),
    Mem(MemTarget),
    Reg16Reg16Offset(Reg16, Reg16),
    MemReg16(MemTarget, Reg16),
    Reg16Imm8(Reg16),
    MemImm8(MemTarget),
    Reset(u8),
}
...
fn ld_rr_rr(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    ...
    let result = cpu.registers.get16(src);
...
fn ld_rr_rr_n8(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    ...
    cpu.registers.set_flag_to(FlagBits::H, (sp & 0x0F) + (offset as u16 & 0x0F) > 0x0F);
...
fn ld_mem_rr(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::MemReg16(mem, src) = instr.operands else { unreachable!() };
...
fn inc_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
...
fn add_r_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegMem(dest, mem) = instr.operands else { unreachable!() };
...
fn adc_r_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegReg(dst, src) = instr.operands else { unreachable!() };

    let carry = cpu.registers.get_flag(FlagBits::C) as u8;

    let reg1 = cpu.registers.get8(src);
...
fn adc_r_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegMem(dst, mem) = instr.operands else { unreachable!() };
...
fn add_rr_n8(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    ...
    cpu.registers.set_flag_to(FlagBits::Z, false);
    cpu.registers.set_flag_to(FlagBits::N, false);
    cpu.registers.set_flag_to(FlagBits::H, (reg1 & 0x0F) + (offset as u16 & 0x0F) > 0x0F);
    cpu.registers.set_flag_to(FlagBits::C, (reg1 & 0xFF) + (offset as u16 & 0xFF) > 0xFF);
...
fn sub_r_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegMem(dst, mem) = instr.operands else { unreachable!() };
...
fn cp_r_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegMem(dst, mem) = instr.operands else { unreachable!() };
...
fn and_r_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegMem(dst, mem) = instr.operands else { unreachable!() };
...
fn sbc_r_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::RegReg(dst, src) = instr.operands else { unreachable!() };
...
fn scf(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    cpu.registers.set_flag_to(FlagBits::N, false);
    cpu.registers.set_flag_to(FlagBits::H, false);
...
fn ccf(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    cpu.registers.set_flag_to(FlagBits::N, false);
    cpu.registers.set_flag_to(FlagBits::H, false);
...
fn cpl(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
...
fn rrc_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    ...
    let old = cpu.registers.get8(reg);
    let carry = (old & 0x01) != 0;
...
fn rrca(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
...
fn rrc_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Mem(mem) = instr.operands else { unreachable!() };
...
fn rl_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Reg(dst) = instr.operands else { unreachable!() };
...
fn rlc(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Reg(dst) = instr.operands else { unreachable!() };

    let reg = cpu.registers.get8(dst);
...
fn rlca(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let reg = cpu.registers.get_a();
...
fn sla_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    ...
    // println!("SLA, REG in: {reg:08b}, out: {result:08b}, carry: {carry}");
...
fn sla_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Mem(mem) = instr.operands else { unreachable!() };
...
fn sra_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Reg(src) = instr.operands else { unreachable!() };
...
fn swap_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Reg(src) = instr.operands else { unreachable!() };
...
fn swap_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::Mem(mem) = instr.operands else { unreachable!() };
...
fn bit_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::BitReg(bit, src) = instr.operands else { unreachable!() };

    let reg = cpu.registers.get8(src);
    let zero = ((1 << bit) & reg) == 0;
...
fn bit_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::BitMem(bit, mem) = instr.operands else { unreachable!() };
...
fn res_r(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::BitReg(bit, src) = instr.operands else { unreachable!() };
...
fn res_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::BitMem(bit, mem) = instr.operands else { unreachable!() };
...
fn set_mem(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::BitMem(bit, mem) = instr.operands else { unreachable!() };
...
fn set_reg(instr: &Instruction, cpu: &mut Cpu, bus: &mut Bus) -> u8 {
    let Operands::BitReg(bit, reg) = instr.operands else { unreachable!() };

    let value = cpu.registers.get8(reg);
    let result= value | (1 << bit);
...
fn operands_from_raw(mnemonic: &str, operands: &Vec<RawOperand>) -> Operands {
    ...
    match oper_tuple {
        ...
        (OperandKind::Reg16(reg1), OperandKind::Reg16(reg2)) => {
            if reg2 == Reg16::SP(Increment) {
                Operands::Reg16Reg16Offset(reg1, reg2)
            } else {
                Operands::Reg16Reg16(reg1, reg2)
            }
        },
        (OperandKind::Mem(mem), OperandKind::Reg16(reg2)) => Operands::MemReg16(mem, reg2),
        (OperandKind::Reg16(reg1), OperandKind::Imm8) => Operands::Reg16Imm8(reg1),
        (OperandKind::Mem(mem), OperandKind::Imm8) => Operands::MemImm8(mem),
        (OperandKind::Reset(vector), OperandKind::None) => Operands::Reset(vector),
...
fn operand_from_raw(operand: &RawOperand, as_condition: bool) -> OperandKind {
    ...
    let base = match operand.name.as_str() {
        ...
        "$00" | "$08" | "$10" | "$18" | "$20" | "$28" | "$30" | "$38" => {
            let string = operand.name.replace("$", "");
            OperandKind::Reset(u8::from_str_radix(&string, 16).expect("Non integer value parsed"))
        },
...
fn dispatch_for(opcode: u8, is_cb: bool, mnemonic: &String, operands: Operands) -> fn(&Instruction, &mut Cpu, &mut Bus) -> u8 {
    match (mnemonic.as_str() ,operands) {
        ...
        ("AND", Operands::RegMem(_, _)) => and_r_mem,
        ...
        ("RETI", Operands::None) => reti,

        ("RST", Operands::Reset(_)) => rst,
        ...
        ("RES", Operands::BitReg(_, _)) => res_r,
        ("RES", Operands::BitMem(_, _)) => res_mem,
